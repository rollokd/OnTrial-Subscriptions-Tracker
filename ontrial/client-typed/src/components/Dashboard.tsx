...
const Dashboard = ({
  ...
  const applySortAndFilter = useCallback((data: Subscription[]) => {
    let result = data;

    // Filter
    if (filterCriteria !== 'all') {
      result = result.filter(sub => filterCriteria === 'active' ? sub.status : !sub.status)
    }
    ...
    result.sort((a, b) => {
      switch (sortCriteria) {
        ...
        case 'billDate':
          return (new Date(a.billingDate)).valueOf() - (new Date(b.billingDate)).valueOf();
        case 'mostExpensive':
